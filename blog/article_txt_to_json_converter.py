...
dict1 = {'name': 'ACME', 'shares': 100, 'price': 542.23}
dict2 = {'name': 'DSSZ', 'shares': 10, 'price': 2.23}

list = [dict1, dict2]



def next_non_empty_lines(lines, current_line_number):
...
def main():
    ...
    with open(article_txt_file_name, 'r') as f:
        ...
        current_line_number = next_non_empty_lines(article_lines, -1) # skip the first empty lines and initialize the current line number

        dict = { 'class'    : 'article-title',
                 'content'  : article_lines[current_line_number][:-1]}
        json_list.append(dict)
        current_line_number = next_non_empty_lines(article_lines, 0) # get to the first relevant line
        ...
        while not article_lines[current_line_number].startswith('#end'): # while the current line is not the end of the article
            ...
            json_list.append(dict)
            current_line_number = next_non_empty_lines(article_lines, current_line_number)
