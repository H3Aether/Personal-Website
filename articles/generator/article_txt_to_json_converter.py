...
def next_non_empty_line(lines, current_line_number):
...
def main():
    ...
    with open(article_txt_file_name, 'r') as f:
        article_lines = f.readlines() # read the lines of the file
        number_of_lines = len(article_lines) # get the number of lines
        current_line_number = next_non_empty_line(article_lines, -1) # skip the first empty lines and initialize the current line number

        dict = { 'class'    : 'article-title',
                 'content'  : article_lines[current_line_number][:-1]}
        json_list.append(dict)

        current_line_number = next_non_empty_line(article_lines, 0) # go the date line
        dict = { 'class'    : 'date',
                 'content'  : article_lines[current_line_number][:-1]}
        json_list.append(dict)

        current_line_number = next_non_empty_line(article_lines, current_line_number) # go the first relevant line
        while not article_lines[current_line_number].startswith('#end'): # while the current line is not the end of the article
            ...
            json_list.append(dict)
            current_line_number = next_non_empty_line(article_lines, current_line_number)
