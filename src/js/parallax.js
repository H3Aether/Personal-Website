...
function desktopParallax(event) {
    const mouseX = event.pageX / window.innerWidth;
    const mouseY = event.pageY / window.innerHeight;
    const offsetX = (0.5 - mouseX) * window.innerWidth * 0.01;
    const offsetY = (0.5 - mouseY) * window.innerWidth * 0.01;
...
function mobileParallax() {
    ...
    parallaxElements.forEach(pel => {
        if (elementInViewport(pel.element)) {
            const x = pel.initial_x;
            const y = pel.initial_y + ((scrollTop-pel.scroll_height*0.88) * pel.sensitivity * 0.1);
            pel.element.style.transform = `translate(${x}px, ${y}px)`;
        }
    });
}
...
const mobile_browser = mobileCheck();
if (!mobile_browser){
    document.addEventListener("mousemove", desktopParallax);
}
// var scrollTop = 0;
// document.addEventListener("scroll", mobileParallax);